...
enum PasswordResetStep { phoneVerify, codeVerify, setNewPassword, completed }
...
class _FindPasswordScreenState extends State<FindPasswordScreen> {
  ...
  PasswordResetStep _currentStep = PasswordResetStep.phoneVerify;
  ...
  Future<void> _sendCode() async {
    ...
    try {
      ...
      if (result.isSuccess) {
        setState(() => _currentStep = PasswordResetStep.codeVerify);
  ...
  Future<void> _verifyCode() async {
    ...
    try {
      ...
      if (result.isSuccess) {
        setState(() => _currentStep = PasswordResetStep.setNewPassword);
  ...
  Future<void> _resetPassword() async {
    ...
    try {
      ...
      if (result.isSuccess) {
        setState(() => _currentStep = PasswordResetStep.completed);
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                if (_currentStep == PasswordResetStep.phoneVerify)
                  _buildPhoneVerifyStep()
                else if (_currentStep == PasswordResetStep.codeVerify)
                  _buildCodeVerifyStep()
                else if (_currentStep == PasswordResetStep.setNewPassword)
  ...
  Widget _buildInstructions() {
    String text;
    switch (_currentStep) {
      case PasswordResetStep.phoneVerify:
        text = '회원가입 시 입력하신 전화번호와 아이디를 입력해주세요. (전화번호는 하이픈 없이 입력)';
        break;
      case PasswordResetStep.codeVerify:
        text = '전화번호로 발송된 인증코드를 입력해주세요.';
        break;
      case PasswordResetStep.setNewPassword:
